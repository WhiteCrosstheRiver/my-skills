...
1. **Deep search**: read [search.md](references/search.md). Expand the topic into several substantive query axes and synonyms, then run `deep-search`. Read all candidate metadata, select relevant papers with reasons, `resume --selection`, read full evidence and author a note for **every** included paper using [notes.md](references/notes.md). Notes are enabled by default; do not stop at an import count.
